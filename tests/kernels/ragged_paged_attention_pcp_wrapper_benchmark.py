...
def make_pcp_wrapper(pcp, tp, chunk, max_ctx):
    ...
    # Scale KV heads if they are fewer than tp_size to prevent IndivisibleError
    tp_size = 1
    for axis in ShardingAxisName.KV_HEAD:
        if axis in mesh.shape:
            tp_size *= mesh.shape[axis]
    nkv_g = NKV
    if tp_size > 1 and nkv_g < tp_size:
        assert tp_size % nkv_g == 0
        nkv_g = tp_size
    nkv2 = align_to(2 * nkv_g, KVP)
    ...
    q = _rand((chunk, NQ, HD))
    k = _rand((chunk, nkv_g, HD))
    v = _rand((chunk, nkv_g, HD))
